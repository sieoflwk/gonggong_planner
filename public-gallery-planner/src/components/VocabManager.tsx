        ...
        <Card title="나만의 단어장" icon={<LanguageIcon className="w-6 h-6" />} className="animate-card-pop-in" style={{ animationDelay: '200ms', opacity: 0 }}>
            <form onSubmit={handleAddWord} className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:items-center">
                ...
                <div className="flex gap-2">
                    ...
                    />
                    <button type="submit" className="flex-shrink-0 bg-brand-primary-500 text-white font-bold p-2.5 rounded-lg hover:bg-brand-primary-600 transition-all transform hover:scale-105 disabled:opacity-50" disabled={!eng.trim() || !kor.trim()}>
            ...
            <div className="min-h-[100px]">
                 {customWords.length > 0 ? (
                    <ul className="space-y-2 max-h-80 overflow-y-auto pr-2">
                        {customWords.map((word, index) => {
                            ...
                            return (
                                <li 
                                    key={word.id} 
                                    className={`flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg group transition-all duration-300 ${isDeleting ? 'animate-list-item-exit overflow-hidden' : 'animate-slide-up-fade'}`}
                                ...
                                >
                                    ...
                                    <button onClick={() => handleDeleteWord(word.id)} className="p-1.5 text-slate-500 hover:text-red-500 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-opacity opacity-0 group-hover:opacity-100">
                    ...
                    <EmptyState 
                        ...
                        title="단어장 비어있음"
                        description="새로운 단어를 추가하여 암기를 시작해보세요."
                    />
                )}
            </div>
        </Card>
