    ...
    return (
        <Card title="오늘의 할 일" icon={<CheckCircleIcon className="w-6 h-6" />} className="animate-card-pop-in" style={{ animationDelay: '100ms', opacity: 0 }}>
            <form onSubmit={handleAddTodo} className="flex gap-2">
                ...
                />
                <button type="submit" className="flex-shrink-0 bg-brand-primary-500 text-white font-bold p-2.5 rounded-lg hover:bg-brand-primary-600 transition-colors disabled:opacity-50" disabled={!newTodoText.trim()}>
                ...
                {todos.length > 0 ? (
                    todos.map((todo, index) => {
                        ...
                        return (
                            <li 
                                key={todo.id} 
                                className={`flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg group transition-all duration-300 ${isDeleting ? 'animate-list-item-exit overflow-hidden' : 'animate-slide-up-fade'}`}
                            ...
                            >
                                ...
                                <button onClick={() => handleDeleteTodo(todo.id)} className="p-1.5 text-slate-500 hover:text-red-500 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 transition-opacity opacity-0 group-hover:opacity-100">
                    ...
                    <p className="text-center text-slate-500 dark:text-slate-400 py-4">오늘 할 일이 없습니다.</p>
                )}
            </ul>
        </Card>
    );
