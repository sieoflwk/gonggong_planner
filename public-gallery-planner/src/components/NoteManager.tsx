...
export const NoteManager: React.FC<NoteManagerProps> = ({ notes, setNotes, subjects, showToast, showModal }) => {
    ...
    return (
        <Card title="오답노트" icon={<DocumentTextIcon className="w-6 h-6" />} className="animate-card-pop-in" style={{ animationDelay: '300ms', opacity: 0 }}>
            <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                <select
                    value={subjectId}
                ...
                <div className="flex items-center gap-4">
                    <label htmlFor="note-image-upload" className="cursor-pointer px-4 py-2 text-sm font-semibold bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors">
                        사진 첨부
                    </label>
                ...
                <div className="flex gap-2">
                    <button type="submit" className="w-full px-4 py-2 font-bold text-white bg-brand-primary-500 rounded-lg hover:bg-brand-primary-600 transition-colors">
                        {editingNote ? '노트 수정' : '노트 저장'}
                    </button>
                    {editingNote && (
                        <button type="button" onClick={resetForm} className="w-full px-4 py-2 font-bold bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors">
                            취소
                        </button>
                    )}
                </div>
            ...
            <ul className="space-y-4 mt-4 max-h-[500px] overflow-y-auto pr-2">
                {filteredNotes.length > 0 ? (
                    filteredNotes.map((note, index) => {
                        ...
                        return (
                            <li 
                                key={note.id} 
                                className={`p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg space-y-3 transition-all duration-300 ${isDeleting ? 'animate-list-item-exit overflow-hidden' : 'animate-slide-up-fade'}`}
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="flex gap-1.5">
                                        <button onClick={() => handleEdit(note)} className="p-1.5 text-slate-500 hover:text-brand-primary-500 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"><EditIcon className="w-4 h-4" /></button>
                                        <button onClick={() => handleDelete(note.id)} className="p-1.5 text-slate-500 hover:text-red-500 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"><TrashIcon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                    ...
                    <p className="text-center text-slate-500 dark:text-slate-400 py-4">{notes.length === 0 ? '등록된 오답노트가 없습니다.' : '검색 결과가 없습니다.'}</p>
                )}
            </ul>
        </Card>
    );
