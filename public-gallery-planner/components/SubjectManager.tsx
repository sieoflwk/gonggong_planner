...
const SubjectItem: React.FC<{
    ...
    return (
        <li 
            className={`flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg transition-all duration-300 ${isDeleting ? 'animate-list-item-exit overflow-hidden' : 'animate-slide-up-fade'}`}
        ...
        >
            {isEditing ? (
                ...
                <div className="flex items-center gap-3 flex-grow min-w-0">
                    ...
                    <span className="text-sm font-medium text-slate-500 dark:text-slate-400 bg-slate-200 dark:bg-slate-600 px-2.5 py-0.5 rounded-full">{subject.count}회독</span>
                </div>
            )}
            <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                 <button onClick={() => onDecrement(subject.id)} className="px-2 py-0.5 text-xs font-bold text-slate-600 bg-slate-200 rounded-md hover:bg-slate-300 dark:text-slate-200 dark:bg-slate-600 dark:hover:bg-slate-500 transition-colors">-1</button>
                 ...
                 <button onClick={() => setIsEditing(true)} className="p-1.5 text-slate-500 hover:text-brand-primary-500 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"><EditIcon className="w-4 h-4" /></button>
                 <button onClick={() => onDelete(subject.id)} className="p-1.5 text-slate-500 hover:text-red-500 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"><TrashIcon className="w-4 h-4" /></button>
            </div>
        </li>
    );
};
    ...
    return (
        <Card title="과목 관리" icon={<BookOpenIcon className="w-6 h-6" />} className="animate-card-pop-in">
            <form onSubmit={handleAddSubject} className="flex gap-2">
                ...
                />
                <button type="submit" className="flex-shrink-0 bg-brand-primary-500 text-white font-bold p-2.5 rounded-lg hover:bg-brand-primary-600 transition-colors disabled:opacity-50" disabled={!newSubjectName.trim()}>
            ...
            <ul className="space-y-2 max-h-80 overflow-y-auto pr-2">
                {subjects.length > 0 ? (
                    ...
                    <p className="text-center text-slate-500 dark:text-slate-400 py-4">등록된 과목이 없습니다.</p>
                )}
            </ul>
        </Card>
    );
